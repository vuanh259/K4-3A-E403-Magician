...
| 5 | ③ Ngoài phạm vi | Điền lệnh `/summary` quét trúng tin học viên hỏi bài trong kênh: *"Bot ơi giải thích cho mình cơ chế Attention trong Transformer với?"* (TC05) | AI trích nhầm câu hỏi thành task bài tập cần làm, tạo thẻ rác vào Action Digest | AI nhận diện là câu hỏi học tập ngoài phạm vi (Out-of-scope), đặt `action_required: false` và bỏ qua không tạo thẻ. Đồng thời danh mục lệnh Discord không có lệnh hỏi bài, hướng dẫn dùng VLearn Tutor | **HAX G1** (Rõ năng lực hệ thống) & **PAIR Mental Models** |
| 6 | ③ Ngoài thẩm quyền | Điền lệnh `/summary` quét trúng tin học viên hỏi: *"Check giúp mình xem hôm nay mình đã được điểm danh chưa bot?"* (TC06) | AI suy đoán bừa dữ liệu điểm danh cá nhân hoặc tạo task tra điểm danh (vượt quyền) | AI nhận diện ngoài thẩm quyền dữ liệu, lọc bỏ qua không tạo task. Trên Discord không có lệnh điểm danh cá nhân | **HAX G1** & **PAIR User Control** |
...
- **Khi gặp nội dung ngoài phạm vi (Out-of-scope — ③):**
  - *Thao tác trên Discord:* Học viên tìm kiếm lệnh hỏi bài/điểm danh bằng cách gõ `/` (ví dụ gõ `/hoi_bai` hoặc `@bot giải thích Transformer`), hoặc khi chạy `/summary` quét phải tin nhắn học viên hỏi bài trong kênh thảo luận.
  - *Xử lý & Kết quả:* 
    1. Trên giao diện lệnh Discord: Danh mục lệnh chỉ hiển thị 4 lệnh hỗ trợ digest (`/summary`, `/tasks`, `/done`, `/correct`), không hỗ trợ lệnh hỏi bài hay tra cứu điểm danh cá nhân, giúp định hình đúng ranh giới năng lực ngay từ đầu (HAX G1).
    2. Trên luồng AI phân tích tin nhắn: Khi quét qua tin hỏi bài học hoặc hỏi điểm danh trong kênh, AI tự động phân loại `action_required: false` và bỏ qua, tuyệt đối không tạo thẻ task rác vào bản tin Action Digest.
